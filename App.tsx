...
import { NumerologySection, DocumentSection, GeneralAISection, ContactSection, HoroscopeSection, BlackMagicSection, WazaifSection, TimeScienceSection, SettingsSection, DreamInterpretationSection, UnaniHealthSection } from './components/FeatureComponents';
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-gradient-to-br from-emerald-900 via-teal-900 to-emerald-950 flex flex-col items-center justify-center text-white p-6 animate-fade-in">
      ...
      <div className="relative z-10 flex flex-col items-center text-center max-w-md w-full">
        ...
        <h1 className="text-2xl md:text-4xl font-bold mb-2 tracking-wide drop-shadow-lg ur-text">طبی و روحانی معالج</h1>
        <p className="text-lg text-emerald-100 font-light leading-relaxed mb-8 opacity-90 ur-text">جدید ٹیکنالوجی اور قدیم علوم کا حسین امتزاج</p>
        <button onClick={onEnter} className="w-full max-w-xs bg-emerald-500 hover:bg-emerald-400 text-white text-xl font-black py-4 px-8 rounded-[1.5rem] shadow-2xl shadow-emerald-950 flex items-center justify-center gap-3 active:scale-95 transition-all ur-text">
          آئیے شروع کریں <ArrowRight size={20} className="rotate-180" />
        </button>
      </div>
    </div>
  );
...
const App: React.FC = () => {
  ...
  const renderSection = () => {
    switch (currentSection) {
      ...
      case AppSection.DreamInterpretation: return <DreamInterpretationSection onBack={goBack} />;
...
const HomeDashboard = ({ onNavigate }: { onNavigate: (s: AppSection) => void }) => {
  ...
  return (
    <div className="space-y-4 pb-8 animate-fade-in-up">
      <div className="bg-gradient-to-br from-emerald-900 to-teal-900 dark:from-slate-900 dark:to-slate-800 rounded-[1rem] p-4 text-white text-center shadow-md relative overflow-hidden border border-emerald-800 dark:border-slate-700">
         <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/arabesque.png')] opacity-5"></div>
         <div className="relative z-10">
            <h2 className="text-lg font-black mb-0.5 drop-shadow-md ur-text">بسم اللہ الرحمٰن الرحیم</h2>
            <p className="text-sm opacity-90 font-bold text-emerald-100 ur-text">طبی و روحانی مسائل کا جدید حل</p>
         </div>
      </div>

      <div className="space-y-3">
           <h3 className="text-base font-black text-gray-800 dark:text-slate-200 mr-1 flex items-center gap-2 ur-text">
              <span className="w-1 h-4 bg-emerald-500 rounded-full"></span>
              طبی خدمات (Medical)
           </h3>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-3xl mx-auto">
              {medicalItems.map((card) => (
                <div key={card.id} onClick={() => onNavigate(card.id)} className="bg-white dark:bg-slate-900 p-4 rounded-[1rem] border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all cursor-pointer flex flex-col items-center text-center gap-3 group">
                   <div className={`p-3 rounded-lg ${card.bg} ${card.color} shadow-inner group-hover:scale-105 transition-transform`}><card.icon size={28} /></div>
                   <div>
                      <h3 className="font-black text-lg text-gray-800 dark:text-white mb-0.5 ur-text">{card.title}</h3>
                      <p className="text-gray-500 dark:text-slate-400 text-[10px] font-bold ur-text">{card.desc}</p>
                   </div>
                   <div className="text-emerald-600 font-black flex items-center gap-1 text-[10px] opacity-0 group-hover:opacity-100 transition-opacity ur-text">
                      دیکھیں <ArrowRight size={12} className="rotate-180" />
                   </div>
                </div>
              ))}
           </div>
           
           <div className="mt-6 p-4 bg-emerald-50/50 dark:bg-slate-900/50 rounded-[1.5rem] border border-dashed border-emerald-200 dark:border-slate-700 text-center">
                <Menu className="mx-auto text-emerald-300 mb-1.5" size={24} />
                <h4 className="text-sm font-black text-emerald-900 dark:text-emerald-400 mb-0.5 ur-text">دیگر تمام روحانی خدمات</h4>
                <p className="text-emerald-700 dark:text-slate-400 text-xs ur-text">اوپر موجود <span className="font-black">مینو</span> سے دیگر تمام خدمات حاصل کریں۔</p>
           </div>
      </div>
    </div>
  );
